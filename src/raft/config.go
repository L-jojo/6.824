...
func (cfg *config) connect(i int) {
	...
	DPrintfWte("[Raft :%d] has been connected", i)
}
...
func (cfg *config) disconnect(i int) {
	...
	DPrintfWte("[Raft :%d] has been disconnected", i)
}
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		//	if cfg.rafts[i].state == 0 {
		//		rf := cfg.rafts[i]
		//		for j := 0; j < len(cfg.rafts); j++ {
		//			if j == i {
		//				continue
		//			}
		//			DPrintfWte("[Leader %d]'s NextIndex[%d] is %d; MatchIndex[%d] is %d ;", rf.me, j, rf.nextIndex[j], j, rf.matchIndex[j])
		//		}
		//	}
		//}
		index := -1
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				//DPrintfWte("cur index : %d server num is %d", index, nd)
