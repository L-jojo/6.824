...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("%d has been disconnected", leader1)
	cfg.checkOneLeader()
	...
	cfg.connect(leader1)
	DPrintf("%d has been reconnected", leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no new leader should
	// be elected.
	cfg.disconnect(leader2)
	DPrintf("%d has been disconnected", (leader2))
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf("%d has been disconnected", (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	...
	DPrintf("%d has been reconnected", (leader2+1)%servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	DPrintf("%d has been reconnected", leader2)
	cfg.checkOneLeader()
	cfg.end()
}
...
func TestFailAgree2B(t *testing.T) {
	...
	//DPrintfWte("servers:%d disconnected", (leader+1)%servers)
	// the leader and remaining follower should be
	// able to agree despite the disconnected follower.
	cfg.one(102, servers-1, false)
	cfg.one(103, servers-1, false)
	...
	// on new commands.
	//DPrintfWte("servers:%d reconnected", (leader+1)%servers)
	time.Sleep(400 * time.Millisecond)
	cfg.one(106, servers, true)
	//DPrintfWte("106 is done")
	time.Sleep(RaftElectionTimeout)
	cfg.one(107, servers, true)
	//DPrintfWte("107 is done")
	cfg.end()
}
